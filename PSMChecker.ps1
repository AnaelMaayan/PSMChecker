...
if ($IsAdmin) {
    ...
    if ($CHECK_WEB_APPS) {
        ...
        $stepsCounter++
        Write-Host "Step $stepsCounter) Web Driver is not updated." -ForegroundColor Yellow
        DriverAndBrowserVersion
        Write-Host ""

        $stepsCounter++
        Write-Host "Step $stepsCounter) UAC is enabled on the PSM." -ForegroundColor Yellow
        UAC
        Write-Host ""
